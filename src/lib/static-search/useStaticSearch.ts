...
export function useStaticSearch(): UseStaticSearchResult {
  const [statusMessage, setStatusMessage] = useState<string | null>('準備中です...')
  ...
  const [duckdbWasmVersion, setDuckdbWasmVersion] = useState<string | null>(null)
  ...
  const executeSearch = useCallback(async (input: string) => {
    ...
    try {
      ...
      console.error(cause)
      setError(cause instanceof Error ? cause.message : '検索中にエラーが発生しました。')
    } finally {
      if (currentSearchId === searchIdRef.current && isActiveRef.current) {
        setIsSearching(false)
      }
